#!/usr/bin/env python3
"""
BuildAny Deploy Script - Fix Build Orchestration
Fixes: Build fires before Morgan finishes, next/document imports, preview generation
Pushed to: https://github.com/taqihas1/buildany-deploy/main/fix_build_orchestration.py
"""
import os, re, json
...
# ============================================================================
# 2. FIX MORGAN-GENERATE - Enhanced sanitization + build after chat
# ============================================================================
generate_path = os.path.join(BUILDANY_DIR, "src/app/api/morgan-generate/route.ts")
with open(generate_path) as f:
    gen_content = f.read()

# Replace sanitizeGeneratedFiles with robust version
old_sanitize = r'function sanitizeGeneratedFiles\(projectDir: string\) \{[\s\S]*?\n\}'

new_sanitize = '''function sanitizeGeneratedFiles(projectDir: string) {
  try {
    const fs = require("fs");
    ...
    walkDir(srcDir, (fp: string) => {
      // Skip _document files - they ARE allowed to import from next/document
      if (fp.includes("_document")) return;
      
      let code = fs.readFileSync(fp, "utf8");
      if (!code.includes("next/document")) return;
      
      console.log(`[Sanitize] Fixing next/document imports in: ${fp}`);
      ...
      // Replace JSX tags with standard HTML equivalents
...
with open(generate_path, "w") as f:
    f.write(gen_content)
print("✅ 2. morgan-generate route.ts - Enhanced sanitization")
...
if os.path.exists(config_file):
    ...
    # Ensure output: 'export' and distDir: 'out' are set
...
# Find and replace the build trigger logic
# The key change: DON'T auto-build on prompt submit
# Instead, build only when Morgan's response has shouldBuild: true

# Check if there's an auto-build on submit
if "handleSubmit" in ws and "setBuildStatus('building')" in ws:
    # We need to modify the chat response handler to trigger build
    # Look for where chat messages are processed
    if "shouldBuild" not in ws:
        # Add build trigger detection in the chat stream/message handler
        ws = ws.replace(
            "const data = await response.json();",
            '''const data = await response.json();
    ...
    print("⚠️ 4. Workspace3Col build logic not found - check manually")
...
if os.path.exists(preview_path):
  ...
  const outPath = path.join(PROJECTS_DIR, projectId, "out", "index.html");
    ...
    return new NextResponse(
      ...
      <html><head><style>
        ...
        <div class="box">
          ...
          <p>Morgan is generating your app. This may take 1-2 minutes.</p>
          <p>Status: <span id="status">Building...</span></p>
